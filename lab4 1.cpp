...
double f(double x,double y) {
  return pow(2.718,(-(x*x+y*y)))*(a*pow(x,2)+b*pow(y,2));
}

double fd_x(double x,double y) {
  return 2 * a*x * exp(-(y * y + x * x)) - 2 * x * (b*y * y + a*x * x)* exp(-(y * y + x * x));
}

double fd_y(double x,double y) {
  return 2 * b*y * exp(-(y * y + x * x)) - 2 * y * (b*y * y + a*x * x)* exp(-(y * y + x * x));
}
...
int metodaGFP(double &x,double &y,double eps,int &nEvF,int &nEvG) {
  const double alpha=0.1,delta=0.1,gamma=0.5;
